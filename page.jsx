...
import { addTransaction } from './actions'; // Import action từ file riêng

// Ép Next.js luôn render "tươi" trang này, không tạo tĩnh lúc build
export const dynamic = 'force-dynamic';

export default async function HomePage() {
  await connectDB();

  // Lấy dữ liệu và "làm sạch" để tránh lỗi "Unsupported Server Component type"
  const rawData = await Transaction.find().sort({ date: -1 }).limit(20).lean();
  const transactions = JSON.parse(JSON.stringify(rawData));

  const totalsData = await Transaction.aggregate([{ $group: { _id: "$type", total: { $sum: "$amount" } } }]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-3xl border shadow-sm h-fit">
          ...
          <form action={addTransaction} className="space-y-4">
            ...
            <button type="submit" className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-all">Lưu dữ liệu</button>
          </form>
        </div>
