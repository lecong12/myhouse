...
import { addTransaction } from './actions'; // Import action từ file vừa tạo
...
export default async function HomePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-slate-50 min-h-screen font-sans">
      <h1 className="text-3xl font-black mb-8 text-slate-900">MY HOUSE 2026</h1>
      
      {/* Chỉ số tài chính */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        <div className="p-6 bg-white border rounded-3xl shadow-sm">
          <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-1">Vốn vào</p>
          <p className="text-2xl font-black">{income.toLocaleString()}đ</p>
        </div>
        <div className="p-6 bg-white border rounded-3xl shadow-sm">
          <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-1">Đã chi</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-3xl border shadow-sm h-fit">
          ...
          <form action={addTransaction} className="space-y-4">
            <input name="title" placeholder="Nội dung chi/thu" required className="w-full p-3 bg-slate-50 rounded-xl outline-none" />
            ...
            <button type="submit" className="w-full py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-100">Lưu dữ liệu</button>
          </form>
        </div>
